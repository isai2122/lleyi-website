...
function App() {
  ...
  const startDate = useRef(new Date(startDateStr));
  const birthDate = useRef(new Date(birthDateStr));
  ...
  function startEdit(year, existingComment = '') {
    // Permitir editar siempre si es el año con edad 17 (excepción especial)
    const birthdayData = futureBirthdays.current.find(b => b.year === year);
    if (birthdayData && birthdayData.age === 17) {
      // Permitir editar sin restricciones
      setEditingYear(year);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-black via-violet-900 to-black p-6 flex items-start justify-center">
      {/* Notification */}
      {showNotification && (
        <div className="fixed top-6 right-6 z-50 bg-gradient-to-r from-pink-600 to-violet-700 text-white px-6 py-4 rounded-lg shadow-2xl flex items-center gap-3 animate-slide-in">
      ...
      <div className="w-full max-w-6xl bg-[rgba(0,0,0,0.45)] rounded-2xl p-6 shadow-2xl border border-[rgba(255,255,255,0.03)]">
        <div className="flex items-center gap-4 mb-4">
          <div className="w-14 h-14 rounded-lg bg-gradient-to-br from-pink-500 to-violet-600 flex items-center justify-center font-bold text-black text-2xl">Y</div>
          <div>
            <h1 className="text-2xl font-extrabold text-white">Tarjeta animada — {name}</h1>
            <p className="text-sm text-slate-300">Animación limpia y lista de cumpleaños hasta 100 años con comentarios permanentes</p>
          </div>
          <div className="ml-auto flex items-center gap-3">
            {!notificationsEnabled && (
              <button
                onClick={enableNotifications}
                className="flex items-center gap-2 px-4 py-2 rounded-md bg-gradient-to-r from-yellow-500 to-orange-600 text-white font-semibold shadow-lg hover:shadow-xl transition-all"
        ...
        {dateCheckMessage && (
          <div className={`mb-4 p-3 rounded-lg text-sm ${canEdit ? 'bg-green-900/30 text-green-300 border border-green-700/50' : 'bg-orange-900/30 text-orange-300 border border-orange-700/50'}`}>
            {dateCheckMessage}
          </div>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {/* LEFT: Visual area */}
          <div className="md:col-span-1 bg-[rgba(255,255,255,0.01)] rounded-xl p-4 relative overflow-hidden flex flex-col items-center">
            <div className="w-full h-36 flex items-center justify-center px-4 mb-4">
              <pre className="text-xl md:text-2xl font-semibold text-transparent bg-clip-text bg-gradient-to-r from-pink-400 to-violet-300 leading-tight text-center whitespace-pre-wrap mb-2">
                {typedText || ' '}
            ...
            {showPhoto && photoUrl ? (
              <img src={photoUrl} alt={name} className="w-64 h-64 rounded-md object-cover shadow-lg overflow-hidden" />
            ) : (
              <div className="w-64 h-64 rounded-md flex items-center justify-center bg-gradient-to-br from-pink-700 to-violet-600 text-white shadow-inner relative overflow-hidden">
            ...
            <div className="mt-3 text-sm text-slate-300">Pulsa <button className="underline hover:text-pink-400" onClick={handleReplay}>Reproducir</button> para repetir</div>
          </div>
          ...
          <div className="md:col-span-2 flex flex-col gap-4">
            <div className="flex items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-2">
                <button onClick={handleReplay} className="px-3 py-2 rounded-md bg-gradient-to-r from-pink-500 to-violet-600 text-black font-semibold shadow hover:shadow-lg transition-all">Reproducir</button>
                <button onClick={() => window.print()} className="px-3 py-2 rounded-md border border-[rgba(255,255,255,0.06)] text-slate-200 hover:bg-white/5 transition-all">Imprimir</button>
              </div>
            </div>

            <div className="bg-[rgba(255,255,255,0.01)] rounded-lg p-3">
              <div ref={listRef} className="h-[480px] overflow-auto pr-4" style={{position: 'relative'}}>
                <div className="space-y-3">
                  {futureBirthdays.current.map((b, i) => {
                    ...
                    return (
                      <div key={i} className="birthday-item p-4 rounded-lg bg-[rgba(255,255,255,0.02)] border border-[rgba(255,255,255,0.03)] transition-all hover:bg-[rgba(255,255,255,0.04)]">
                        <div className="flex justify-between items-start mb-2">
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            {!hasComment && canEdit && b.year === new Date().getFullYear() && (
                              <button
                                onClick={() => startEdit(b.year)}
                            ...
                            {hasComment && canEdit && b.year === new Date().getFullYear() && (
                              <button
                                onClick={() => startEdit(b.year, hasComment.comment)}
                        ...
                        {hasComment && !isEditing && (
                          <div className="mt-2 p-3 bg-[rgba(0,0,0,0.3)] rounded-md border-l-4 border-pink-500">
                        ...
                        {/* Edit form */}
                        {isEditing && (
                          <div className="mt-2 p-3 bg-[rgba(0,0,0,0.4)] rounded-md border border-pink-500/50">
                            <textarea
                              value={commentText}
                              onChange={(e) => setCommentText(e.target.value)}
                              placeholder="¿Cómo estuvo el cumpleaños?"
                              className="w-full h-20 p-2 rounded-md bg-[rgba(255,255,255,0.05)] text-sm text-white resize-none outline-none border border-[rgba(255,255,255,0.1)] focus:border-pink-500"
                              autoFocus
                            />
                            <div className="flex gap-2 mt-2">
                              <button
                                onClick={() => saveComment(b.year, b.age)}
                                className="flex-1 px-3 py-2 rounded-md bg-gradient-to-r from-pink-500 to-violet-600 text-white font-semibold inline-flex items-center justify-center gap-2 hover:shadow-lg transition-all"
                              >
                                <Check className="w-4 h-4" /> Guardar
                              </button>
                              <button
                                onClick={cancelEdit}
                                className="px-3 py-2 rounded-md border border-[rgba(255,255,255,0.1)] text-slate-300 hover:bg-white/5 transition-all"
                              >
                                Cancelar
                              </button>
                            </div>
              ...
              <div className="absolute right-6 top-[180px] flex flex-col gap-2">
                <button onClick={() => scrollList(-180)} className="w-9 h-9 rounded-md bg-white/5 text-white hover:bg-white/10 transition-all">▲</button>
                <button onClick={() => scrollList(180)} className="w-9 h-9 rounded-md bg-white/5 text-white hover:bg-white/10 transition-all">▼</button>
              </div>
            </div>
            ...
            {/* final message */}
            {showBirthdays && (
              <div className="bg-gradient-to-r from-pink-600 to-violet-700 text-white p-4 rounded-lg text-center animate-fade-in">
      ...
      <div className="pointer-events-none fixed inset-0 -z-10">
        {[...Array(12)].map((_, i) => (
          <Heart key={i} className="absolute text-pink-500/8" style={{ left: `${(i * 13) % 100}%`, top: `${(i * 21) % 100}%`, width: `${20 + (i % 5) * 8}px`, transform: `translateY(${(i % 2 ? -8 : 6)}px)` }} />
        ))}
      </div>
    </div>
  );
